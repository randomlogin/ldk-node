...
pub(crate) async fn generate_blocks_and_wait<E: ElectrumApi>(
	...
	let new_tip_height = cur_height as usize + num;
	wait_for_block(electrs, new_tip_height).await;
	print!(" Done!");
	println!("\n");

	bitcoind
		.get_block_hash(new_tip_height as u64)
		.expect("failed to get block hash")
		.block_hash()
		.expect("block hash should be present")
}

/// Poll bitcoind's mempool until it contains at least `min_count` txs, or panic
/// after `timeout`. Call this before `generate_blocks_and_wait` at sites where
/// a tx broadcast is expected, so the broadcast has time to traverse
/// `tx_broadcaster → kyoto → P2P → bitcoind` before the next block is mined.
pub(crate) async fn wait_for_mempool_size(
	bitcoind: &BitcoindClient, min_count: usize, timeout: Duration,
) {
	...
	loop {
		let mempool = bitcoind
			...
			.into_model()
			.expect("failed to parse mempool");
		if mempool.0.len() >= min_count {
			return;
		}
		if start.elapsed() > timeout {
			panic!(
				"wait_for_mempool_size: timed out waiting for >= {} txs (mempool size: {})",
				min_count,
				mempool.0.len(),
			);
		}
...
/// Poll `node.status().current_best_block.block_hash` until it equals `expected`,
/// or panic after `timeout`. Useful with CBF chain sources where there's a P2P
/// propagation delay between bitcoind mining a block and the node seeing it.
pub(crate) async fn wait_for_node_tip(
	node: &TestNode, expected: bitcoin::BlockHash, timeout: Duration,
) {
	let start = std::time::Instant::now();
	let mut delay = Duration::from_millis(100);
	loop {
        if node.status().current_best_block.block_hash == expected {
            tokio::time::sleep(Duration::from_secs(2)).await;
            return;
		}
		if start.elapsed() > timeout {
			panic!(
				"wait_for_node_tip: timed out waiting for {} (current best block: {})",
				expected,
...
pub(crate) async fn do_channel_full_cycle<E: ElectrumApi>(
	...
	if !allow_0conf {
        ...
        wait_for_node_tip(&node_a, tip, wait_timeout).await;
        wait_for_node_tip(&node_b, tip, wait_timeout).await;
	}
	...
	if force_close {
		...
		// node_b's sweeper has scheduled a sweep tx broadcast triggered by the
		// confirmation just observed. Wait for it to land in bitcoind's mempool
		// so the next block we mine actually includes it.
		wait_for_mempool_size(&bitcoind, 1, wait_timeout).await;
		let tip = generate_blocks_and_wait(&bitcoind, electrsd, 1).await;
		wait_for_node_tip(&node_b, tip, wait_timeout).await;
		wait_for_node_tip(&node_a, tip, wait_timeout).await;
		node_b.sync_wallets().unwrap();
		...
		// Mirror of the node_b path above: wait for node_a's sweep tx to land in
		// the mempool before mining, so the block actually includes it.
		wait_for_mempool_size(&bitcoind, 1, wait_timeout).await;
		let tip = generate_blocks_and_wait(&bitcoind, electrsd, 1).await;
		wait_for_node_tip(&node_a, tip, wait_timeout).await;
		wait_for_node_tip(&node_b, tip, wait_timeout).await;
		...
		let tip = generate_blocks_and_wait(&bitcoind, electrsd, node_a_blocks_to_go as usize).await;
        wait_for_node_tip(&node_a, tip, wait_timeout).await;
        wait_for_node_tip(&node_b, tip, wait_timeout).await;
		node_a.sync_wallets().unwrap();
		node_b.sync_wallets().unwrap();
